  	...
  	for (l=0; l<ibm[ibi].n_elmt; l++) {
			...
			*/
			...
			if (deltafunc == 0) {
				...
				dfunc = dfunc_sc4h(r1) * dfunc_sc4h(r2) * dfunc_sc4h(r3);
			}
			*/
	            	...
	            	//InterfaceCell += dfunc;          
//        PetscPrintf(PETSC_COMM_WORLD, "IPlagr: %le %le %i\n", ibm[ibi].U_IPlagr_z[l],dfunc,ibi);
         
//       if(ibi>0){
//          printf( "Ucat_Dfunc: %le %i %i %i %i \n", ucat[k][j][i].x, k,j,i,my_rank);
//        }
//
      		}
...
//       if(ibi>0){
//          printf( "Ulocal_IPlagr: %le %le %i %i %i \n", u_local[i].x ,ibm[ibi].U_IPlagr_x[i],i,ibm[ibi].n_elmt, my_rank);
//       }
  		}
...
//        PetscPrintf(PETSC_COMM_WORLD, "USUM_IPlagr: %le %i %i %i\n", u_sum[i].z ,ibm[ibi].n_elmt, count_vector, ibi);
...
// 	  PetscBarrier(NULL);
//    PetscPrintf(PETSC_COMM_WORLD, "Nacelle Yaw %le %le %le \n",nx,ny,nz);
//    PetscPrintf(PETSC_COMM_WORLD, "Nacelle Yaw Old %le %le %le \n",nxo,nyo,nzo);
...
// 	   PetscBarrier(NULL);
//     PetscPrintf(PETSC_COMM_WORLD, "Matrix Mult 2222\n");
...
// 	PetscBarrier(NULL);
//  PetscPrintf(PETSC_COMM_WORLD, "Rotation Matrix done \n");
//
  Pre_process(&(user[0]), ibm, NumberOfObjects);

// 	PetscBarrier(NULL);
//  PetscPrintf(PETSC_COMM_WORLD, "PreProcess done \n");
//
  Coordinates_IP(ibm, NumberOfObjects);

// 	PetscBarrier(NULL);
//  PetscPrintf(PETSC_COMM_WORLD, "Coordinates IP done \n");

//There is a here in this subroutine
  Pre_process_IP(&(user[0]), ibm, NumberOfObjects);

// 	PetscBarrier(NULL);
//  PetscPrintf(PETSC_COMM_WORLD, "PreProcess IP done \n");
