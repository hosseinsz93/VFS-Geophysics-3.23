...
extern int pseudo_periodic, rans, implicit, conv_diff,sandwave;
extern PetscInt block_number, freesurface, immersed, ti, tistart, les, tiout, sediment, density_current, levelset;
...
PetscInt lidx(PetscInt i, PetscInt j, PetscInt k, UserCtx *user)
{
	...
	if (!(user->aotopetsc)) {
		...
		ISLocalToGlobalMappingGetIndices(ltog, (const PetscInt**)&user->idx_from);
	}
...
/*   MatNullSpaceDestroy(&nullsp); */
  ...
  if (!user->multinullspace) {
    ...
    //MPIU_Allreduce(&lsum, &sum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
    VecSum(X, &sum);
    MPIU_Allreduce(&lnum, &num, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
    sum = sum / (-1.0 * num);
...
PetscErrorCode PoissonSolver_MG(UserMG *usermg, IBMNodes *ibm, IBMInfo *ibminfo)
{
	...
	extern PetscInt sediment,movefsi, rotatefsi;
	if(sediment || movefsi || rotatefsi) {
  ...
  for (bi=0; bi<block_number; bi++) {
	if(was_ksp_set != 10) {
		...
		PetscOptionsInsertString(NULL, "-pc_hypre_type boomeramg");
		//PCHYPRESetType(pc, "parasails");	// works
		//PCHYPRESetType(pc, "pilut");
		
		PetscPrintf(PETSC_COMM_WORLD, "KSPSetType...\n");
		MatStructure flag;
		if(sediment, movefsi || rotatefsi) flag=DIFFERENT_NONZERO_PATTERN;
...
PetscErrorCode PoissonSolver_MG_original(UserMG *usermg, IBMNodes *ibm, IBMInfo *ibminfo)
{
  ...
  if( ti==tistart || movefsi || rotatefsi || levelset || sediment )
  for (l=usermg->mglevels-1; l>=0; l--) {
    user = mgctx[l].user;
    for (bi=0; bi<block_number; bi++) {
...
/*     PetscOptionsGetBool(NULL, NULL, "-after_mg", &flg, NULL); */
/*     if (flg) { */
...
/*     } */
  ...
  for (l=usermg->mglevels-1; l>=0; l--) {
    ...
    for (bi=0; bi<block_number; bi++) {
      ...
      if( sediment || movefsi || rotatefsi || levelset ) MatDestroy(&user[bi].A);
...
PetscErrorCode VolumeFlux(UserCtx *user, Vec lUcor, PetscReal *ibm_Flux, PetscReal *ibm_Area)
{
  ...
  MPIU_Allreduce(&libm_Flux, ibm_Flux, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&libm_area, ibm_Area, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
...
PetscErrorCode VolumeFlux(UserCtx *user, PetscReal *ibm_Flux, PetscReal *ibm_Area, PetscInt flg)
{
  ...
  MPIU_Allreduce(&libm_Flux, ibm_Flux, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&libm_Flux_abs, &ibm_Flux_abs, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&libm_area, ibm_Area, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
...
PetscErrorCode Do_averaging(UserCtx *user)
{
	...
	PetscReal ***p2sum, ***p, ***k_sum, ***lnu_t, ***nut_sum,  ***aj, ***nvert, ***udpsum, ***taussum, ***level, ***level_sum, ***level2sum, ***conc, ***conc_sum, ***ustar, ***ustar_sum, ***ustar_;
	Cmpnts2 ***ko;
	...
	//VecAXPY(user->Conc_sum, 1., user->Conc); 
	...
	DMDAVecGetArray(user->da, user->lP, &p);
        ...
        if(sandwave){
	DMDAVecGetArray(user->da, user->lUstar, &ustar);
	DMDAVecGetArray(user->da, user->lUstar_sum, &ustar_sum);
	DMDAVecGetArray(user->da, user->lUstar_, &ustar_);
	}

        if(conv_diff){
	DMDAVecGetArray(user->da, user->Conc, &conc);
	DMDAVecGetArray(user->da, user->Conc_sum, &conc_sum);
	}

        if(levelset){
	DMDAVecGetArray(user->da, user->lLevelset, &level);
	DMDAVecGetArray(user->da, user->Level_sum, &level_sum);
	DMDAVecGetArray(user->da, user->Level_square_sum, &level2sum);
	}
	  ...
	  double U = ucat[k][j][i].x, V = ucat[k][j][i].y, W = ucat[k][j][i].z;
		...
		if(les) {
			nut_sum[k][j][i] += lnu_t[k][j][i];
                ...
                if(sandwave)  {
                        ustar_sum[k][j][i] += ustar[k][j][i];
                        ustar_[k][j][i] = ustar[k][j][i];
                 }

                if(conv_diff)  {
                        conc_sum[k][j][i] += conc[k][j][i];
                 }

                if(levelset)  {
                        level_sum[k][j][i] += level[k][j][i];
                        level2sum[k][j][i] += level[k][j][i]  *  level[k][j][i];
                        //level2sum[k][j][i] += (level[k][j][i] + cent[k][j][i].z) * (cent[k][j][i].z + level[k][j][i]);
                 }
	...
	DMDAVecRestoreArray(user->da, user->Conc_sum, &conc_sum);
	}
        
        if(levelset) {
	DMDAVecRestoreArray(user->da, user->lLevelset, &level);
	DMDAVecRestoreArray(user->da, user->Level_sum, &level_sum);
	DMDAVecRestoreArray(user->da, user->Level_square_sum, &level2sum);
	}

        if(sandwave) {
	DMDAVecRestoreArray(user->da, user->lUstar, &ustar);
	DMDAVecRestoreArray(user->da, user->lUstar_sum, &ustar_sum);
	DMDAVecRestoreArray(user->da, user->lUstar_, &ustar_);
	}
...
PetscErrorCode KE_Output(UserCtx *user)
{
	...
	for (j=lys; j<lye; j++)
	for (i=lxs; i<lxe; i++) {
		//Hossein modidfied based on wave code
	...
	MPIU_Allreduce(&local_sum, &sum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
