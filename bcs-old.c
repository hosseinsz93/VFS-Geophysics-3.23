...
extern int initial_perturbation1;
...
{
  PetscReal u;
  //PetscPrintf(PETSC_COMM_WORLD, "Inlet at r=%f\n", r);
...
// Hossein-7/15/2025 (Verdant-SBU project) - Open Channel Inlet Profile: Logaritmic (arbitrary depth)
PetscReal InletInterpolationLISTEP(PetscReal xinlet0, PetscReal yinlet0, PetscReal r, UserCtx *user)  
{
  // Read loglaw_borders.txt (comma-separated)
	std::vector<PetscReal> X_surf, Y_surf, Z_surf;
	std::ifstream file("loglaw_borders.txt");

	if (!file.is_open()) {
		PetscPrintf(PETSC_COMM_WORLD, "Error: Could not open loglaw_borders.txt\n");
		return 0.0;
	}
	...
	while (std::getline(file, line)) {
		...
		if (iss >> x >> comma >> y >> comma >> z && comma == ',') {
			...
			PetscPrintf(PETSC_COMM_WORLD, "Warning: Skipping malformed line: %s\n", line.c_str());
		}
	}
	file.close();

	// Find the two closest points
	PetscInt closest_idx1 = -1, closest_idx2 = -1;
	PetscReal min_dist1 = 10000000, min_dist2 = 10000000;

	for (size_t i = 0; i < X_surf.size(); i++) {
		PetscReal dx = xinlet0 - X_surf[i];
		PetscReal dy = yinlet0 - Y_surf[i];
		PetscReal dist_sq = dx * dx + dy * dy;
	...
	PetscReal x1 = X_surf[closest_idx1], y1 = Y_surf[closest_idx1], z1 = Z_surf[closest_idx1];
	PetscReal x2 = X_surf[closest_idx2], y2 = Y_surf[closest_idx2], z2 = Z_surf[closest_idx2];

	// Compute weights based on inverse distance (closer point has more influence)
	PetscReal dist1 = sqrt(min_dist1);  // Actual distance to closest point
	PetscReal dist2 = sqrt(min_dist2);  // Actual distance to second closest point
	PetscReal total_dist = dist1 + dist2;
	PetscReal weight1 = (total_dist > 0) ? (dist2 / total_dist) : 0.5;  // Avoid division by zero
	PetscReal weight2 = (total_dist > 0) ? (dist1 / total_dist) : 0.5;

	// Interpolated point
	PetscReal x_interp = weight1 * x1 + weight2 * x2;
	PetscReal y_interp = weight1 * y1 + weight2 * y2;
	PetscReal z_interp = weight1 * z1 + weight2 * z2;

	//// --- Print Results ---
	//PetscPrintf(PETSC_COMM_WORLD, "Closest points:\n");
	//PetscPrintf(PETSC_COMM_WORLD, "  Point 1 (index %d): X = %.6f, Y = %.6f, Z = %.6f\n",closest_idx1, x1, y1, z1);
	//PetscPrintf(PETSC_COMM_WORLD, "  Point 2 (index %d): X = %.6f, Y = %.6f, Z = %.6f\n",closest_idx2, x2, y2, z2);
	//PetscPrintf(PETSC_COMM_WORLD, "Interpolated point: X = %.6f, Y = %.6f, Z = %.6f\n",x_interp, y_interp, z_interp);
	 
		// --- Original velocity profile logic ---	
	PetscReal u;
  
	r = abs(r);

    if(r-z_interp <=0.001746) u = 1.e-8;
    else if((r-z_interp) >0.001746 && (r-z_interp) <=0.02237) u = 0.2389 * log(r-z_interp) + 1.5128;
    else if((r-z_interp) >0.02237 && (r-z_interp) <=0.8) u = 0.1408 * log(r-z_interp) + 1.1592;
    else u = 0.1408 * log(0.8) + 1.1592; 

	if (r < z_interp) u = 1.e-8;
	//printf("Coordinate: %f for velocity of %f\n", r, u);
	...
	if (user->bctype[4] == INLET) {
		if (zs==0) {
			...
			for (i=lxs; i<lxe; i++) {
				...
				//if (inletprofile==99) uin = InletInterpolationLISTEP(xc, yc, zc, user);
		...
		*/			
		...
		if (ze==mz) {
			...
			for (i=lxs; i<lxe; i++) {
				...
				//else if (inletprofile==99) {
					//uin = InletInterpolationLISTEP(xc, yc, zc, user);
				//}
...
double generate_fluctuation(int i, int j, int k, double length_scale) {
    double dx=0.01;
	double dy=0.01;
	double dz=0.01;
	// Use spatial coordinates to generate correlated random values
    double x = i * dx;  // Assume dx is your grid spacing
    double y = j * dy;
    double z = k * dz;
    
    // Generate correlated fluctuations using multiple frequency components
    double fluctuation = 0.0;
    int num_modes = 10;  // Number of frequency modes
    
    for (int m = 0; m < num_modes; m++) {
        // Generate random phase and direction for each mode
        double phase_x = 2.0 * M_PI * (rand() / (double)RAND_MAX);
        double phase_y = 2.0 * M_PI * (rand() / (double)RAND_MAX);
        double phase_z = 2.0 * M_PI * (rand() / (double)RAND_MAX);
        
        // Wave number inversely proportional to length scale
        double kappa = (m + 1) / length_scale;
        
        // Add contribution from this mode
        fluctuation += sin(kappa * x + phase_x) * 
                       sin(kappa * y + phase_y) * 
                       sin(kappa * z + phase_z) / (m + 1);
    }
...
PetscErrorCode SetInitialGuessToOne(UserCtx *user)
{
	...
	double turbulence_intensity = 0.126;  // 5% turbulence intensity
	double length_scale = 0.111;           // Integral length scale (in grid units)
	double u_mean = 0.85;                 // Mean velocity magnitude

	// Calculate fluctuation amplitude based on turbulence intensity
	double u_rms = turbulence_intensity * u_mean;

// Replace random perturbations with a more controlled approach
	...
	/*if (initial_perturbation1) {
		PetscPrintf(PETSC_COMM_WORLD, "\nGenerating initial perturbation with TI=%.2f%%, L=%.2f\n", TI*100, L);
	
		// Parameters
		const double U_mean = 1.0;          // Mean velocity in z-direction
		const double u_rms = TI * U_mean;   // Root-mean-square fluctuation
		const double dx = 0.01;             // Grid spacing (adjust to your mesh)
		const double sigma = L / 2.0;       // Gaussian filter width (tune to match L)
		const int radius = (int)(3*sigma/dx); // Filter radius (~3σ)
		
		// Temporary noise storage (3D arrays for x,y,z components)
		// Allocate these arrays based on your grid dimensions
		
		double ***noise_x, ***noise_y, ***noise_z;
		
		// ... Initialize 3D arrays ...
	
		// Generate white noise (parallel RNG recommended - this is simplified)
		for (k = lzs; k < lze; k++) {
			for (j = lys; j < lye; j++) {
				for (i = lxs; i < lxe; i++) {
					noise_x[k][j][i] = ((double)rand()/RAND_MAX)*2.0 - 1.0;
					noise_y[k][j][i] = ((double)rand()/RAND_MAX)*2.0 - 1.0;
					noise_z[k][j][i] = ((double)rand()/RAND_MAX)*2.0 - 1.0;
				}
			}
		}
		PetscPrintf(PETSC_COMM_WORLD, "\nHi4");
	
		// Apply Gaussian filter and set perturbations
		for (k = lzs; k < lze; k++) {
			for (j = lys; j < lye; j++) {
				for (i = lxs; i < lxe; i++) {
					if (nvert[k][j][i] + nvert[k+1][j][i] < threshold) {
						...
						for (int dk = -radius; dk <= radius; dk++) {
							for (int dj = -radius; dj <= radius; dj++) {
								for (int di = -radius; di <= radius; di++) {
									// Use ghost/halo cells for parallel correctness
									int kk = k + dk, jj = j + dj, ii = i + di;
									if (kk < lzs || kk >= lze || 
										jj < lys || jj >= lye || 
										ii < lxs || ii >= lxe) continue;
	
									double r = sqrt(di*di + dj*dj + dk*dk)*dx;
									double weight = exp(-r*r/(2*sigma*sigma));
									
									sum_x += noise_x[kk][jj][ii] * weight;
									sum_y += noise_y[kk][jj][ii] * weight;
									sum_z += noise_z[kk][jj][ii] * weight;
									weight_sum += weight;
								}
							}
						}
						...
						// Apply perturbations
						ucont[k][j][i].x = sum_x * u_rms;
						ucont[k][j][i].y = sum_y * u_rms;
						ucont[k][j][i].z = U_mean + sum_z * u_rms;
					}
				}
			}
		}
		...
		// ... Cleanup noise_x, noise_y, noise_z ...
	}*/

	// Create spatially correlated random field
	
	/*if(initial_perturbation1) {
		PetscPrintf(PETSC_COMM_WORLD, "\nGenerating initial perturbation with TI=%g%%, L=%g\n", 
					turbulence_intensity*100, length_scale);
		...
		for(k=lzs; k<lze; k++) {
			for(j=lys; j<lye; j++) {
				for(i=lxs; i<lxe; i++) {
					if(nvert[k][j][i]+nvert[k+1][j][i] < threshold) {
						// Base velocity
						double u_base = ucont[k][j][i].z;
						
						// Generate spatially correlated fluctuations
						double u_fluc = u_rms * generate_fluctuation(i, j, k, length_scale);
						double v_fluc = u_rms * generate_fluctuation(i+100, j+100, k+100, length_scale);
						double w_fluc = u_rms * generate_fluctuation(i+200, j+200, k+200, length_scale);
						
						// Apply fluctuations with proper scaling
						ucont[k][j][i].x += u_fluc * icsi[k][j][i].x;
						ucont[k][j][i].y += v_fluc * jeta[k][j][i].y;
						ucont[k][j][i].z += w_fluc * kzet[k][j][i].z;
					}
				}
			}
		}
	...
	double Ti = 0.05;
	if(initial_perturbation1) {
		PetscPrintf(PETSC_COMM_WORLD, "\nGenerating initial perturbation with TI=%f\n", turbulence_intensity);
		for(k=lzs; k<lze; k++) {
			for (j=lys; j<lye; j++)
			for (i=lxs; i<lxe; i++) {
				if (nvert[k][j][i]+nvert[k+1][j][i] < threshold) {
					int n1, n2, n3;
					double base_velocity = ucont[k][j][i].z; // Base velocity magnitude
					
					// Generate random numbers with normal distribution
					// These approximate normal distribution using multiple random numbers
					double sum1 = 0, sum2 = 0, sum3 = 0;
					for(int r=0; r<12; r++) {
						sum1 += (double)(rand())/RAND_MAX;
						sum2 += (double)(rand())/RAND_MAX;
						sum3 += (double)(rand())/RAND_MAX;
					}
					double rand1 = (sum1-6.0)/sqrt(3.0); // Approx N(0,1)
					double rand2 = (sum2-6.0)/sqrt(3.0); // Approx N(0,1)
					double rand3 = (sum3-6.0)/sqrt(3.0); // Approx N(0,1)
					
					// Apply turbulence intensity
					ucont[k][j][i].x = turbulence_intensity * base_velocity * rand3;
					ucont[k][j][i].y = turbulence_intensity * base_velocity * rand2;
					ucont[k][j][i].z = base_velocity * (1 + turbulence_intensity * rand1);
				}
			}
		}
	}
