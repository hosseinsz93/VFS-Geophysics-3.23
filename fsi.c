...
PetscErrorCode FsiInitialize(PetscInt n_elmt, FSInfo *fsi,PetscInt ibi)
{
  /*  Note: This routine shouldn't be called before ibm_read */
   PetscPrintf(PETSC_COMM_WORLD, "Called FsiInitialize function in fsi.c for IB %i\n\n", ibi);
  ...
  fsi->red_vel=0.52;//1.5;//0.3921;///KFlora check out why these values are hard coded in
  fsi->damp=.02;
  ...
  PetscPrintf(PETSC_COMM_WORLD, "From FsiInitilize, IBM %i Axis:%i at XYAngle:%f\n", ibi, fsi->rot_dir,fsi->XYangle);
  PetscPrintf(PETSC_COMM_WORLD, "From FsiInitilize, Angles Rotate: %f %f %f\n", fsi->S_ang_n[0], fsi->S_ang_n[2], fsi->S_ang_n[4]);
  PetscPrintf(PETSC_COMM_WORLD, "From FsiInitilize, Center of Rot: %f %f %f\n", fsi->x_c, fsi->y_c, fsi->z_c);
  PetscPrintf(PETSC_COMM_WORLD, "Combined Values, IBM %i Axis:%i at XYAngle:%f and Coord. Angles Rotate: %f %f %f, \nCenter of Rot: %f %f %f\n", ibi, fsi->rot_dir,fsi->XYangle ,fsi->S_ang_n[0], fsi->S_ang_n[2], fsi->S_ang_n[4], fsi->x_c, fsi->y_c, fsi->z_c);
/*   Check Later!!!!!!!!!!!!!!!!
  //fsi->y_c += 0.5;
  //fsi->x_c += 0.5;
 // fsi->z_c +=ibi*1.5;
...
/* 		 elmtinfo[elmt].kcell,rank,pc.x,pc.y,pc.z);  */
  ...
  MPIU_Allreduce(&Aroundcellnum, &AroundCellSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
...
/* 	} */
    ...
    for (nbn=0; nbn<n_elmt; nbn++) {
      MPIU_Allreduce(&n_Pr[nbn], &n_Psum[nbn], 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
      MPIU_Allreduce(&Ps[nbn], &PsSum[nbn], 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
      MPIU_Allreduce(&Tow_ws[nbn], &Tow_wsSum[nbn], 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
      MPIU_Allreduce(&Tow_wt[nbn], &Tow_wtSum[nbn], 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
    }
...
/*     MPIU_Allreduce(&F_z, &F_ztotal, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD); */
/*     MPIU_Allreduce(&F_y, &F_ytotal, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD); */
...
/*   PetscMalloc(n_elmt*sizeof(PetscReal), &ns_z); */
  ...
  F_y=Cp_y + Cs_y;
  F_z=Cp_z + Cs_z;
  PetscPrintf(PETSC_COMM_WORLD, "\nCalc_force x => Cp: %d, Cs=%d, Fx=%d\n %d\n",Cp_x, Cs_x, F_x);
  
  // Global Sum
  MPIU_Allreduce(&F_x, &F_xSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&F_y, &F_ySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&F_z, &F_zSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&A_tot, &A_totSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);

  MPIU_Allreduce(&Cp_x, &Cp_xSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Cp_y, &Cp_ySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Cp_z, &Cp_zSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
...
/*   M_z=M_z/A_tot*2; */
  ...
  // Global Sum
  MPIU_Allreduce(&M_x, &M_xSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&M_y, &M_ySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&M_z, &M_zSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&A_tot, &A_totSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
...
/*   PetscPrintf(PETSC_COMM_SELF, "CV fluxes, %le %le %le mass %le\n", flux_y, Fp_y,Ft_y, massflux); */
  ...
  MPIU_Allreduce(&F_y, &F_ySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&F_z, &F_zSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Finert_z, &Finert_zSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Finert_y, &Finert_ySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Fint_z, &Fint_zSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Fint_y, &Fint_ySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&massflux, &MassFluxSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Finv_z, &Finv_zSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Finv_y, &Finv_ySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Fvis_z, &Fvis_zSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Fvis_y, &Fvis_ySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
...
/* ==================================================================================             */  
/*   Global Sum */
  MPIU_Allreduce(&F_px, &F_pxSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&F_py, &F_pySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&F_pz, &F_pzSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);

  MPIU_Allreduce(&F_nx, &F_nxSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&F_ny, &F_nySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&F_nz, &F_nzSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);

  MPIU_Allreduce(&Ap_x, &Ap_xSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Ap_y, &Ap_ySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Ap_z, &Ap_zSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);

  MPIU_Allreduce(&An_x, &An_xSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&An_y, &An_ySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&An_z, &An_zSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);

  MPIU_Allreduce(&Cp_nx, &Cp_nxSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Cp_ny, &Cp_nySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Cp_nz, &Cp_nzSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);

  MPIU_Allreduce(&Cp_px, &Cp_pxSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Cp_py, &Cp_pySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Cp_pz, &Cp_pzSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);

  MPIU_Allreduce(&M_px, &M_pxSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&M_py, &M_pySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&M_pz, &M_pzSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);

  MPIU_Allreduce(&M_nx, &M_nxSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&M_ny, &M_nySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&M_nz, &M_nzSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);

  MPIU_Allreduce(&Iap_x, &Iap_xSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Iap_y, &Iap_ySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Iap_z, &Iap_zSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);

  MPIU_Allreduce(&Ian_x, &Ian_xSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Ian_y, &Ian_ySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Ian_z, &Ian_zSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);

  MPIU_Allreduce(&Mdpdn_px, &Mdpdn_pxSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Mdpdn_py, &Mdpdn_pySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Mdpdn_pz, &Mdpdn_pzSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);

  MPIU_Allreduce(&Mdpdn_nx, &Mdpdn_nxSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Mdpdn_ny, &Mdpdn_nySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Mdpdn_nz, &Mdpdn_nzSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);

  MPIU_Allreduce(&Pw_px, &Pw_pxSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Pw_py, &Pw_pySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Pw_pz, &Pw_pzSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);

  MPIU_Allreduce(&Pw_nx, &Pw_nxSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Pw_ny, &Pw_nySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Pw_nz, &Pw_nzSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);

  MPIU_Allreduce(&Ap_t, &Ap_tSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&An_t, &An_tSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);

  MPIU_Allreduce(&Thrust, &ThrustSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Drag, &DragSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&Pw_side, &Pw_sideSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
