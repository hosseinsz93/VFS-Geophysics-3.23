...
void Grad_phi_criterion_Dennis (UserCtx *user)
{	
//	
//	=============================
//	Dionysios Angelidis 7/12/2018
//	=============================
//	
	...
	PetscInt	wall_distance;	
	PetscReal	dpdc, dpde, dpdz, min_grad, max_grad, tolerance;
	PetscScalar	this_cpu_flag, all_cpus_flag, grad_phi_now;
	double dldc, dlde, dldz, dl_dx, dl_dy, dl_dz, csi0, csi1, csi2, eta0, eta1, eta2, zet0, zet1, zet2, ajc, dx, sgn;
	Cmpnts		***csi,	***eta,	***zet;
	PetscReal	***aj, ***level, ***level0,	***rhs, ***nvert;
...
//
//	
	tolerance = 0.01;		//<-------- Selected tolerance for the convergence criterion
//
//
	this_cpu_flag = 1;
//	
...
//
		...
		ajc = aj[k][j][i];
//
		dx=pow(1./aj[k][j][i],1./3.);
//
		if(dthick_set) dx = dthick;
//
		sgn = sign1(level0[k][j][i], dx);
...
//
//		Apply criterion only around fluid nodes
//
		if((nvert[k][j][i]+nvert[k+1][j][i]+nvert[k-1][j][i]+nvert[k][j+1][i]+nvert[k][j-1][i]+nvert[k][j][i+1]+nvert[k][j][i-1])<0.1) {
			grad_phi_now = sqrt(dl_dx*dl_dx + dl_dy*dl_dy +	dl_dz*dl_dz);
		} else {
			grad_phi_now = 1; 																	// to facilitate the convergence criterion
		}
//
//		Convergence criterion
//
		if(	(grad_phi_now>(1.+tolerance))  ||  (grad_phi_now<(1.-tolerance))	) 	
			{
				this_cpu_flag = 0;
				//PetscPrintf(PETSC_COMM_SELF,"Non converged grad_phi=%le \n",grad_phi_now);	//Ali deactivated for TexWashBridge
			}
	}
//
		MPI_Allreduce( &this_cpu_flag,  &all_cpus_flag,    1, MPIU_SCALAR, MPI_MIN, MPI_COMM_WORLD );	
//
		if(all_cpus_flag>0.9)	flag_level_set_conv = 1;	// If all all_cpus_flag==1 --> converged
//	
//
//
...
void Grad_phi_criterion_Dennis_00 (UserCtx *user)
{
	flag_level_set_conv = 0;
...
//
	PetscReal	min_grad, max_grad;
//
	VecMax(L,NULL,&max_grad);
	VecMax(L,NULL,&min_grad);
//
	PetscPrintf(PETSC_COMM_SELF,"Level-set : min_grad_phi = %le, max_grad_phi = %le \n",min_grad,max_grad);	
//
//
	PetscReal tolerance;
//
//	1-tolerance < |gradphi| < 1 + tolerance // Max and min values should obey this criterion
//
	tolerance = 0.01; // DENNIS - Ali this is the tolerance for the gradphi criterion
	
	
	if((min_grad>(1.-tolerance))||(min_grad<(1.+tolerance))) 
	if((max_grad>(1.-tolerance))||(max_grad<(1.+tolerance))) flag_level_set_conv = 1; // No more iterations needed
//
	DMDAVecRestoreArray(fda, 	Csi,			&csi	);
	DMDAVecRestoreArray(fda, 	Eta,			&eta	);
	DMDAVecRestoreArray(fda,	Zet,			&zet	);
...
{
	DM		da = user->da, fda = user->fda;
	DMDALocalInfo	info = user->info; //Hossein
				...
				if(solitary_wave){
						...
						double z0 = (coor[to][j][i].z + coor[to][j-1][i].z + coor[to][j][i-1].z + coor[to][j-1][i-1].z) * 0.25;
				                ...
				                double t = user->dt*(double)ti;
                                  	...
                                  	//PetscPrintf(PETSC_COMM_WORLD, "\nFormFunction_Distance:z_to, z, y_to, y:%e %e %e %e\n\n", z_to,z0, y_to,y0);
...
{	
	DM		da = user->da, fda = user->fda; //Hossein
	DMDALocalInfo	info = user->info; //Hossein
	PetscInt	xs,	xe,	ys,	ye,	zs,	ze;	// Local grid	information
	PetscInt	mx,	my,	mz;	// Dimensions	in three directions
	PetscInt	i, j,	k;
				...
				if(solitary_wave){
						...
						double z0 = (coor[to][j][i].z + coor[to][j-1][i].z + coor[to][j][i-1].z + coor[to][j-1][i-1].z) * 0.25;
				                ...
				                double t = user->dt*(double)ti;
                                                double free_surface_elevation = 0.;
                                                double free_surface_elevation_to = 0.;
                                  	//PetscPrintf(PETSC_COMM_WORLD, "\nSolve_Distance_Explicit:z_to, z, y_to, y:%e %e %e %e\n\n", z_to,z0, y_to,y0);
				  if(solitary_wave == 1) Solitary_wave_inlet_elevation_profile_Boussinesq(&user[0], &free_surface_elevation, &free_surface_elevation_to, x0, y_to, z_to, z_to, t, ti);
				  if(solitary_wave == 2) Linear_wave_single_inlet_elevation_profile(&user[0], &free_surface_elevation, &free_surface_elevation_to, x0, y_to, z_to, z_to, t, ti);
				  level[to][j][i] = free_surface_elevation_to - y_to;
                                                 }
