...
extern PetscReal Nseg, U_Bulk, sediment_thickness,d50,deltab, FlowDepth, porosity, sbbb, Cs_, Angle_repose;
        ...
        for( elmt = 0; elmt < n_elmt; elmt++ )
	{
		...
		ibm->cent_yl[ elmt ] = ibm->cent_y[elmt];}
	        ...
	        y_minimum=PetscMin(ibm->y_bp[ibm->nv1[elmt]],ibm->y_bp[ibm->nv2[elmt]]);
                ...
                if(ds_max_ >= 0.4 || (!y_direction && ibm->nf_z[elmt] < 0.8) || (y_direction && ibm->nf_y[elmt] < 0.8)) ibm->elmt_depth[elmt] = 5.;
                if(ibm->elmt_depth[elmt] > 0.4) ibm->elmt_depth[elmt] = 5.;
                ...
                if(!y_direction){if(ibm->cent_z[elmt]<0.99)ibm->elmt_depth[elmt]=5.;
                                 ...
                                 else {if(ibm->cent_y[elmt]<0.4999)ibm->elmt_depth[elmt]=5.;
                                             if(ibm->cent_y[elmt]>0.5001)ibm->elmt_depth[elmt]=5.;}
		        ...
		        ibm->netflux_old[elmt] = netflux;
                       ...
                       // if((ibm->BShS[elmt]-ibm->BCShS[elmt])<=0.) netflux=PetscMax(0., netflux);                      
                     	...
                     	if (!y_direction)
						{
							...
							if(ibm->cent_z[elmt] < 3.) ibm->cent_y[elmt]= ibm->cent_y_old[elmt];
							if(ibm->cent_y_old[elmt]>=0.999)  ibm->cent_y[elmt] = ibm->cent_y_old[elmt]; 
	...
	{       
			 ...
			 if(riter > 0.)
			 {
				if(!y_direction)
				{
						...
						if(ibm->z_bp[vert] <= 3.)ibm->y_bp[vert] = ibm->y_bp_o[vert]; 
