...
PetscErrorCode TECIOOut_V(UserCtx *user, int only_V)	// seokkoo
{
	...
	if(only_V)   {
		...
		else if (levelset && !conv_diff && !rans) I = TECINI100((char*)"Flow", (char*)"X Y Z U V W UU P Nv Level Fr", filen, (char*)".", &Debug, &VIsDouble);
	...
	for (bi=0; bi<block_number; bi++) {
		...
		*/
		...
		if(conv_diff) {
			...
			for (k=zs; k<ze-2; k++)
			for (j=ys; j<ye-2; j++)
			for (i=xs; i<xe-2; i++) x[ (k-zs) * (IMax-1)*(JMax-1) + (j-ys) * (IMax-1) + (i-xs)] = conc[k+1][j+1][i+1];
...
PetscErrorCode TECIOOut_Averaging(UserCtx *user)	// seokkoo
{
	...
	if(pcr) I = TECINI100((char*)"Averaging", "X Y Z P Velocity_Magnitude Nv",  filen, (char*)".",  &Debug,  &VIsDouble);
		...
		if(averaging_option==1) {
			...
			//I = TECINI100((char*)"Averaging", "X Y Z U V W uu vv ww uv vw uw UU_ VV_ WW_ Nv",  filen, (char*)".",  &Debug,  &VIsDouble); //OSL
			 if(levelset)I = TECINI100((char*)"Averaging", "X Y Z U V W uu vv ww uv vw uw l lrms K Nv",  filen, (char*)".",  &Debug,  &VIsDouble);
			 if(!levelset)I = TECINI100((char*)"Averaging", "X Y Z U V W uu vv ww uv vw uw K Nv",  filen, (char*)".",  &Debug,  &VIsDouble);
			...
			I = TECINI100((char*)"Averaging", "X Y Z U V W uu vv ww uv vw uw P pp Vortx Vorty Vortz vortx2 vorty2 vortz2 Nv",  filen, (char*)".",  &Debug,  &VIsDouble);
                      ...
                      if(conv_diff && levelset) I = TECINI100((char*)"Averaging", "X Y Z U V W K C l lrms Nv",  filen, (char*)".",  &Debug,  &VIsDouble);
                      else if(conv_diff) I = TECINI100((char*)"Averaging", "X Y Z U V W K C Nv",  filen, (char*)".",  &Debug,  &VIsDouble);
                      else if(levelset) I = TECINI100((char*)"Averaging", "X Y Z U V W K l lrms Nv",  filen, (char*)".",  &Debug,  &VIsDouble);
                      else I = TECINI100((char*)"Averaging", "X Y Z U V W K Nv",  filen, (char*)".",  &Debug,  &VIsDouble);
	}
	else if(avg==2 && sandwave){
                       I = TECINI100((char*)"Averaging", "X Y Z uf vf wf F ustar ustarf Nv",  filen, (char*)".",  &Debug,  &VIsDouble);
	}
	...
	for (bi=0; bi<block_number; bi++) {
	 ...
	 //PetscPrintf(PETSC_COMM_WORLD, "here 6\n");
			// uv
			for (k=zs; k<ze-2; k++) 
			for (j=ys; j<ye-2; j++) 
			for (i=xs; i<xe-2; i++) {
	 ...
	 //PetscPrintf(PETSC_COMM_WORLD, "here 7\n");
			// k
			for (k=zs; k<ze-2; k++)
			for (j=ys; j<ye-2; j++)
			for(i=xs; i<xe-2; i++) {
	 ...
	 //PetscPrintf(PETSC_COMM_WORLD, "here 8\n");
			if(i_average) Iavg(x, xs, xe, ys, ye, zs, ze, mx, my, mz);
			if(j_average) Javg(x, xs, xe, ys, ye, zs, ze, mx, my, mz);
			if(k_average) Kavg(x, xs, xe, ys, ye, zs, ze, mx, my, mz);
			if(ik_average) IKavg(x, xs, xe, ys, ye, zs, ze, mx, my, mz);
	 ...
	 //PetscPrintf(PETSC_COMM_WORLD, "here 9\n");
			if(levelset){
...
PetscErrorCode ibm_read(IBMNodes *ibm)
{
  ...
  if(!rank) { // root processor read in the data
    FILE *fd;
    fd = fopen("ibmdata0", "r"); if (!fd) SETERRQ(PETSC_COMM_SELF, PETSC_ERR_FILE_OPEN, "Cannot open IBM node file");n_v =0;
    fscanf(fd, "%i", &n_v);
    fscanf(fd, "%i", &n_v);
    fscanf(fd, "%le", &xt);
    ibm->n_v = n_v;
...
PetscErrorCode ibm_read_ucd(IBMNodes *ibm)
{
  ...
  if(!rank) { // root processor read in the data
    FILE *fd;
    fd = fopen("ibmdata", "r");
    if (!fd) SETERRQ(PETSC_COMM_SELF, PETSC_ERR_FILE_OPEN, "Cannot open IBM node file");n_v =0;
