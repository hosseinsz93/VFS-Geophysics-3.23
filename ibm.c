...
/*   insertnode(&test, 11); */
  ...
  // for previous bodies nvert 3 inside, 1 near bndry
	...
	if(rotatefsi && ti>tistart && ibi>NumberOfBodies) {
					  ...
					  PetscInt Add_dUndt)
{
  ...
  if (wallfunction) {
    ...
    MPIU_Allreduce(&us_max, &us_maxSum, 1, MPIU_REAL, MPIU_MAX, PETSC_COMM_WORLD);
    MPIU_Allreduce(&us_min, &us_minSum, 1, MPIU_REAL, MPIU_MIN, PETSC_COMM_WORLD);
...
PetscErrorCode ibm_interpolation_advanced(UserCtx *user)
{
	...
	if(levelset) {
		...
		DMLocalToGlobal (da, user->lDensity, INSERT_VALUES, Density);
		DMLocalToGlobal (da, user->lMu, INSERT_VALUES, Mu);
		...
		for(ibi=0; ibi<NumberOfBodies; ibi++) {
			...
			while (current) {
				  ...
				  ustar[k][j][i]=0;
					...
					if(levelset) {
					  ...
					  if(rough_set && ibi==0/*&& i!=1 && i!=mx-2 && j!=1 && j!=my-2 && k!=1 && k!=mz-2*/){
					    //wall_function_roughness (1./ren, roughness_size, sc, sb, Ua, Uc, &ucat[k][j][i], &ustar[k][j][i], ibm->nf_x[ni], ibm->nf_y[ni], ibm->nf_z[ni]);
					    wall_function_roughness_loglaw (1./ren, roughness_size, sc, sb, Ua, Uc, &ucat[k][j][i], &ustar[k][j][i], ibm->nf_x[ni], ibm->nf_y[ni], ibm->nf_z[ni]);
					  }
				...
				if(averaging) {
					...
					if(wallfunction) wall_function_roughness_a (user, 0.0001, sc, sb, Ua, Uc_avg, &tmp, &ustar_avg, ibm->nf_x[ni], ibm->nf_y[ni], ibm->nf_z[ni]);
