...
void Tmprt_IC(UserCtx *user)
{
	...
	DMLocalToGlobal(user->da, user->lTmprt, INSERT_VALUES, user->Tmprt);
...
void Force_Tmprt(UserCtx *user)
{
	...
	for (j=0; j<my; j++) {
  	...
  	PetscReal     u_sum;
		...
		MPIU_Allreduce(&(tmprt_xzAvg[j]), &u_sum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
...
//		if (k==3 && i==3) printf("the force  %d %d %le %le %le %le\n", my-2, j, ftmprt[k][j][i].y, force_x, force_y, force_z);
	...
	DMLocalToGlobal(user->fda, user->lFTmprt, INSERT_VALUES, user->FTmprt);
...
*/
	...
	DMDAVecRestoreArray(user->da, user->lTmprt, &tmprt);
        ...
        DMLocalToLocalBegin(user->da, user->lTmprt, INSERT_VALUES, user->lTmprt);
        DMLocalToLocalEnd(user->da, user->lTmprt, INSERT_VALUES, user->lTmprt);
...
void Solve_Tmprt(UserCtx *user)
{
	...
	if(!snes_Tmprt_created) {
		...
		SNESSetType(snes_Tmprt_eq, SNESNEWTONTR);			//SNESNEWTONTR,SNESLS	
	...
	SNESMonitorSet(snes_Tmprt_eq,MySNESMonitor,NULL,NULL);
	
	PetscPrintf(PETSC_COMM_WORLD, "\nSolving Temperature...\n");
	
	SNESSolve(snes_Tmprt_eq, NULL, user[bi].Tmprt);
