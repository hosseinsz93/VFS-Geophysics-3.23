...
extern PetscInt block_number, freesurface, immersed, ti, tistart, les, tiout, sediment;
...
void MatHYPRE_IJMatrixCopy(Mat v,HYPRE_IJMatrix &ij)
 ...
 void Remove_Nullspace_Scale(UserCtx *user, HYPRE_IJVector &B, int i_lower)
 {
	...
	MPIU_Allreduce(&localsum, &sum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
	MPIU_Allreduce(&localsum_aj, &sum_aj, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
...
//	extern PetscInt movefsi, rotatefsi;
	...
	else if(sediment || movefsi || rotatefsi || (levelset && !fix_level) ) {
	...
	if(sediment) {flg=1;} else {flg=0;}
	if(immersed==1) VolumeFlux(&user[bi], &ibm_Flux, &ibm_Area, flg);
