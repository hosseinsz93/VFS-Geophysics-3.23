...
PetscReal InletInterpolationLIS(PetscReal xinlet0, PetscReal yinlet0, PetscReal r, UserCtx *user)   //Open Channel Inlet Profile: Logaritmic for Long Island Sound
{	
//#include <vector>
//#include <fstream>
//#include <sstream>
...
PetscReal inletintepolationLIS1(PetscReal xinlet0, PetscReal yinlet0, PetscReal r, UserCtx *user)   // Open Channel Inlet Profile: Power law for Long Island Sound
{
	// Read powerlaw_borders.txt (comma-separated)
	std::vector<PetscReal> X_surf, Y_surf, Z_surf;
	std::ifstream file("powerlaw_borders.txt");

	if (!file.is_open()) {
		PetscPrintf(PETSC_COMM_WORLD, "Error: Could not open powerlaw_borders.txt\n");
		return 0.0;
	}
	...
	if (X_surf.size() < 2) {
		PetscPrintf(PETSC_COMM_WORLD, "Error: Not enough points in powerlaw_borders.txt\n");
		return 0.0;
	}
	...
	if (closest_idx1 < 0 || closest_idx2 < 0) {
		PetscPrintf(PETSC_COMM_WORLD, "Error: Could not interpolate powerlaw_borders.txt points\n");
		return 0.0;
	}

	// Linear interpolation between the two nearest bed points
	PetscReal x1 = X_surf[closest_idx1], y1 = Y_surf[closest_idx1], z1 = Z_surf[closest_idx1];
	PetscReal x2 = X_surf[closest_idx2], y2 = Y_surf[closest_idx2], z2 = Z_surf[closest_idx2];

	PetscReal dist1 = sqrt(min_dist1);
	PetscReal dist2 = sqrt(min_dist2);
	PetscReal total_dist = dist1 + dist2;
	PetscReal weight1 = (total_dist > 0) ? (dist2 / total_dist) : 0.5;
	PetscReal weight2 = (total_dist > 0) ? (dist1 / total_dist) : 0.5;
	...
	PetscReal u;
	PetscReal delta = 1, a = 4.2632; // Ebb = 4.2632, Flood = 6.2991
	PetscReal y_local;

	r = abs(r);
	y_local = r - z_interp;

	if (r < z_interp || y_local <= 0.0) u = 1.e-8;
	else if (y_local >= delta) u = 1.0;
	else u = pow(y_local / delta, 1.0 / a);
	...
	if (user->bctype[4] == INLET) {
		if (zs==0) {
			...
			for (i=lxs; i<lxe; i++) {
				...
				if (inletprofile==0) {
					if (y_direction) uin = InletInterpolation(yc, user);
					else uin = InletInterpolation(zc, user);
				}
				...
				else if (inletprofile == 200) uin = InletInterpolationLIS(xc, zc, yc, user);
				else if (inletprofile == 201) uin = inletintepolationLIS1(xc, zc, yc, user);
		...
		if (ze==mz) {
			...
			/*for (j = lys; j < lye; j++)
				for (i = lxs; i < lxe; i++) {
					...
					printf("Coordinate: %d and %d for velocity of %f\n",i,j,uin);
				}*/
