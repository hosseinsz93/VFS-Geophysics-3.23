...
#include "petscksp.h"
#include "petscpc.h"
extern PetscInt block_number, ti;
extern PetscInt immersed;

PetscErrorCode MyPoissonKSPMonitor(KSP ksp, PetscInt n, PetscReal rnom, void *dummy)
{
  UserCtx *user = (UserCtx*)dummy;
  Vec x;
  PetscInt tttt, mi, i, j, k;
  PetscReal norm;
...
/*     AOApplicationToPetsc(ao, count, user->idx_from); */
    ...
    DMGetLocalToGlobalMapping(user->da, NULL, &user->idx_from);
  }
...
/*   DMLocalToLocalBegin(da, F1, INSERT_VALUES, F1); */
/*   DMLocalToLocalEnd(da, F1, INSERT_VALUES, F1); */
...
PetscErrorCode PoissonLHSNew(UserCtx *user, IBMNodes *ibm, IBMInfo *ibminfo)
{
  ...
  if (!user->assignedA) {
    ...
    MatCreateAIJ(PETSC_COMM_WORLD, M, M, N, N, 19, NULL, 19, NULL, &(user->A));
...
/* 	KSPGetPC(subksp, &subpc); */
/* 	PCSetType(subpc, PCBJACOBI); */
/* 	PetscInt cs_max_it=25; */
/* 	PetscOptionsGetInt(NULL, NULL, "-mg-coarse_ksp_max_it", &cs_max_it, NULL); */
...
/*     //    KSPSetMonitor(mgksp[bi], MyKSPMonitor, &user[bi], NULL); */
...
PetscErrorCode InterpolationFlux(UserCtx *user)
{
  ...
  MPIU_Allreduce(&lFluxIntp, &user->FluxIntpSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  //  MPIU_Allreduce(&lInsideBody, &InsideBodySum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
  MPIU_Allreduce(&lFluidVolume, &FluidVolumeSum, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
