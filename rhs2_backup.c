...
void store_i_section(UserCtx *user, int iplane)
{
	...
	if ( !ucat_plane_allocated_is ) {
		ucat_plane_allocated_is = 1;
		
		user->ucat_plane_is = (Cmpnts **) malloc( sizeof(Cmpnts *) * mz );
		if(rans) user->komega_plane_is = (Cmpnts2 **) malloc( sizeof(Cmpnts2 *) * mz );
		if(temperature) user->tmprt_plane_is = (double **) malloc( sizeof(double *) * mz ); // add xiaolei
		if(temperature1) user->tmprt1_plane_is = (double **) malloc( sizeof(double *) * mz ); // add xiaolei
		if(temperature2) user->tmprt2_plane_is = (double **) malloc( sizeof(double *) * mz ); // add xiaolei
		if(temperature3) user->tmprt3_plane_is = (double **) malloc( sizeof(double *) * mz ); // add xiaolei
		if(levelset) user->level_plane_is = (double **) malloc( sizeof(double *) * mz ); // add xiaolei
		
		for(k=0; k<mz; k++) {
			user->ucat_plane_is[k] = (Cmpnts *) malloc( sizeof(Cmpnts) * my );
			if(rans) user->komega_plane_is[k] = (Cmpnts2 *) malloc( sizeof(Cmpnts2) * my );
			if(temperature) user->tmprt_plane_is[k] = (double *) malloc( sizeof(double) * my ); //add xiaolei
			if(temperature1) user->tmprt1_plane_is[k] = (double *) malloc( sizeof(double) * my ); //add xiaolei
			if(temperature2) user->tmprt2_plane_is[k] = (double *) malloc( sizeof(double) * my ); //add xiaolei
			if(temperature3) user->tmprt3_plane_is[k] = (double *) malloc( sizeof(double) * my ); //add xiaolei
			if(levelset) user->level_plane_is[k] = (double *) malloc( sizeof(double) * my ); //add xiaolei
		}
	}
	...
	double ***tmprt1; // add xiaolei
	double ***tmprt2; // add xiaolei
	double ***tmprt3; // add xiaolei
	double ***level; // add xiaolei

	/*
	std::vector< std::vector<Cmpnts> > ucat_plane_tmp (mz);
	std::vector< std::vector<Cmpnts2> > komega_plane_tmp (mz);
	std::vector< std::vector<double> > tmprt_plane_tmp (mz); // add xiaolei
	std::vector< std::vector<double> > level_plane_tmp (mz); // add xiaolei
	
	for( k=0; k<mz; k++) ucat_plane_tmp[k].resize(mx);
	for( k=0; k<mz; k++) komega_plane_tmp[k].resize(mx);
	for( k=0; k<mz; k++) tmprt_plane_tmp[k].resize(mx); // add xiaolei
	for( k=0; k<mz; k++) level_plane_tmp[k].resize(mx); // add xiaolei
	for(i=0; i<mx; i++) {
		ucat_plane_tmp[k][i].x = ucat_plane_tmp[k][i].y = ucat_plane_tmp[k][i].z = 0;
		komega_plane_tmp[k][i].x = komega_plane_tmp[k][i].y = 0;
		tmprt_plane_tmp[k][i] = 0;  // add xiaolei
		level_plane_tmp[k][i] = 0;  // add xiaolei
	}
	...
	*/
	...
	for(k=0; k<mz; k++) 
	for(j=0; j<my; j++) {
		user->ucat_plane_is[k][j].x = 0;
              	user->ucat_plane_is[k][j].y = 0;
                user->ucat_plane_is[k][j].z = 0;
        }
        if (rans){
	    	for(k=0; k<mz; k++) 
	        for(j=0; j<my; j++) {
		    user->komega_plane_is[k][j].x = 0;
                    user->komega_plane_is[k][j].y = 0;
            	}
	}
        if (temperature){
	    	for(k=0; k<mz; k++) 
	        for(j=0; j<my; j++) {
	 	    user->tmprt_plane_is[k][j] = 0;
		}
	}
        if (temperature1){
	    	for(k=0; k<mz; k++) 
	        for(j=0; j<my; j++) {
	 	    user->tmprt1_plane_is[k][j] = 0;
		}
	}
        if (temperature2){
	    	for(k=0; k<mz; k++) 
	        for(j=0; j<my; j++) {
	 	    user->tmprt2_plane_is[k][j] = 0;
		}
	}
        if (temperature3){
	    	for(k=0; k<mz; k++) 
	        for(j=0; j<my; j++) {
	 	    user->tmprt3_plane_is[k][j] = 0;
		}
	}
	...
	if(temperature1) DMDAVecGetArray(user->da, user->Tmprt1, &tmprt1); // add xiaolei
	if(temperature2) DMDAVecGetArray(user->da, user->Tmprt2, &tmprt2); // add xiaolei
	if(temperature3) DMDAVecGetArray(user->da, user->Tmprt3, &tmprt3); // add xiaolei
	...
	for (k=zs; k<ze; k++)
	for (j=ys; j<ye; j++) {
 		user->ucat_plane_is[k][j] = ucat[k][j][iplane];
	}
        if (rans){
		for (k=zs; k<ze; k++)
		for (j=ys; j<ye; j++) {
			user->komega_plane_is[k][j] = komega[k][j][iplane];
		}
	}
        if (temperature){
		for (k=zs; k<ze; k++)
		for (j=ys; j<ye; j++) {
			user->tmprt_plane_is[k][j] = tmprt[k][j][iplane]; // add xiaolei
		}
	}
        if (temperature1){
		for (k=zs; k<ze; k++)
		for (j=ys; j<ye; j++) {
			user->tmprt1_plane_is[k][j] = tmprt1[k][j][iplane]; // add xiaolei
		}
	}
        if (temperature2){
		for (k=zs; k<ze; k++)
		for (j=ys; j<ye; j++) {
			user->tmprt2_plane_is[k][j] = tmprt2[k][j][iplane]; // add xiaolei
		}
	}
        if (temperature3){
		for (k=zs; k<ze; k++)
		for (j=ys; j<ye; j++) {
			user->tmprt3_plane_is[k][j] = tmprt3[k][j][iplane]; // add xiaolei
		}
	}
		...
		for (k=zs; k<ze; k++)
		for (j=ys; j<ye; j++) {
			user->level_plane_is[k][j] = level[k][j][iplane]; // add xiaolei
		}
	}
	...
	if(levelset) DMDAVecRestoreArray(user->da, user->Levelset, &level); // add xiaolei

        if (!rank) {
		for(k=0; k<mz; k++){ 
			MPI_Reduce(MPI_IN_PLACE, user->ucat_plane_is[k], my*3, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
		}
                if (rans){
			for(k=0; k<mz; k++) {
				MPI_Reduce(MPI_IN_PLACE, user->komega_plane_is[k],  my*2, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature) {
			for(k=0; k<mz; k++) {
				MPI_Reduce(MPI_IN_PLACE, user->tmprt_plane_is[k], my, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature1) {
			for(k=0; k<mz; k++) {
				MPI_Reduce(MPI_IN_PLACE, user->tmprt1_plane_is[k], my, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature2) {
			for(k=0; k<mz; k++) {
				MPI_Reduce(MPI_IN_PLACE, user->tmprt2_plane_is[k], my, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature3) {
			for(k=0; k<mz; k++) {
				MPI_Reduce(MPI_IN_PLACE, user->tmprt3_plane_is[k], my, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}

		if(levelset){
			for(k=0; k<mz; k++) {
 				MPI_Reduce(MPI_IN_PLACE, user->level_plane_is[k], my, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
	}else{
		for(k=0; k<mz; k++){ 
			MPI_Reduce(user->ucat_plane_is[k], user->ucat_plane_is[k], my*3, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
		}
                if (rans){
			for(k=0; k<mz; k++) {
				MPI_Reduce(user->komega_plane_is[k], user->komega_plane_is[k], my*2, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature) {
			for(k=0; k<mz; k++) {
				MPI_Reduce(user->tmprt_plane_is[k], user->tmprt_plane_is[k], my, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature1) {
			for(k=0; k<mz; k++) {
				MPI_Reduce(user->tmprt1_plane_is[k], user->tmprt1_plane_is[k], my, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature2) {
			for(k=0; k<mz; k++) {
				MPI_Reduce(user->tmprt2_plane_is[k], user->tmprt2_plane_is[k], my, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature3) {
			for(k=0; k<mz; k++) {
				MPI_Reduce(user->tmprt3_plane_is[k], user->tmprt3_plane_is[k], my, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}

		if(levelset){
			for(k=0; k<mz; k++) {
 				MPI_Reduce(user->level_plane_is[k], user->level_plane_is[k], my, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
	}
};

void store_j_section(UserCtx *user, int jplane)
{
	DMDALocalInfo info = user->info;
	int	xs = info.xs, xe = info.xs + info.xm;
	int 	ys = info.ys, ye = info.ys + info.ym;
	...
	if ( !ucat_plane_allocated_js ) {
		ucat_plane_allocated_js = 1;
		
		user->ucat_plane_js = (Cmpnts **) malloc( sizeof(Cmpnts *) * mz );
		if(rans) user->komega_plane_js = (Cmpnts2 **) malloc( sizeof(Cmpnts2 *) * mz );
		if(temperature) user->tmprt_plane_js = (double **) malloc( sizeof(double *) * mz ); // add xiaolei
		if(temperature1) user->tmprt1_plane_js = (double **) malloc( sizeof(double *) * mz ); // add xiaolei
		if(temperature2) user->tmprt2_plane_js = (double **) malloc( sizeof(double *) * mz ); // add xiaolei
		if(temperature3) user->tmprt3_plane_js = (double **) malloc( sizeof(double *) * mz ); // add xiaolei
		if(levelset) user->level_plane_js = (double **) malloc( sizeof(double *) * mz ); // add xiaolei
		
		for(k=0; k<mz; k++) {
			user->ucat_plane_js[k] = (Cmpnts *) malloc( sizeof(Cmpnts) * mx );
			if(rans) user->komega_plane_js[k] = (Cmpnts2 *) malloc( sizeof(Cmpnts2) * mx );
			if(temperature) user->tmprt_plane_js[k] = (double *) malloc( sizeof(double) * mx ); //add xiaolei
			if(temperature1) user->tmprt1_plane_js[k] = (double *) malloc( sizeof(double) * mx ); //add xiaolei
			if(temperature2) user->tmprt2_plane_js[k] = (double *) malloc( sizeof(double) * mx ); //add xiaolei
			if(temperature3) user->tmprt3_plane_js[k] = (double *) malloc( sizeof(double) * mx ); //add xiaolei
			if(levelset) user->level_plane_js[k] = (double *) malloc( sizeof(double) * mx ); //add xiaolei
		}
	}
	...
	*/
	...
	for(k=0; k<mz; k++) 
	for(i=0; i<mx; i++) {
		user->ucat_plane_js[k][i].x = 0;
                user->ucat_plane_js[k][i].y = 0;
                user->ucat_plane_js[k][i].z = 0;
        }
        if (rans) {
	    for(k=0; k<mz; k++) 
	    for(i=0; i<mx; i++) {
		user->komega_plane_js[k][i].x = 0;
                user->komega_plane_js[k][i].y = 0;
            }
	}
        if (temperature) {
	    for(k=0; k<mz; k++) 
	    for(i=0; i<mx; i++) {
		user->tmprt_plane_js[k][i] = 0;
	    }
	}		
        if (temperature1) {
	    for(k=0; k<mz; k++) 
	    for(i=0; i<mx; i++) {
		user->tmprt1_plane_js[k][i] = 0;
	    }
	}		
        if (temperature2) {
	    for(k=0; k<mz; k++) 
	    for(i=0; i<mx; i++) {
		user->tmprt2_plane_js[k][i] = 0;
	    }
	}		
        if (temperature3) {
	    for(k=0; k<mz; k++) 
	    for(i=0; i<mx; i++) {
		user->tmprt3_plane_js[k][i] = 0;
	    }
	}		
        if (levelset){ 
	    for(k=0; k<mz; k++) 
	    for(i=0; i<mx; i++) {
		user->level_plane_js[k][i] = 0;
	    }
 	}
	...
	if(levelset) DMDAVecGetArray(user->da, user->Levelset, &level); // add xiaolei
		...
		for (k=zs; k<ze; k++)
		for (i=xs; i<xe; i++){
                    user->ucat_plane_js[k][i] = ucat[k][jplane][i];
		}

        	if (rans) {
			for (k=zs; k<ze; k++)
			for (i=xs; i<xe; i++) {
				user->komega_plane_js[k][i] = komega[k][jplane][i];
			}
		}
			...
			for (k=zs; k<ze; k++)
			for (i=xs; i<xe; i++) {
				user->tmprt_plane_js[k][i] = tmprt[k][jplane][i]; // add xiaolei
			}
		}
			...
			for (k=zs; k<ze; k++)
			for (i=xs; i<xe; i++) {
				user->tmprt1_plane_js[k][i] = tmprt1[k][jplane][i]; // add xiaolei
			}
		}
	        if (temperature2){
			for (k=zs; k<ze; k++)
			for (i=xs; i<xe; i++) {
				user->tmprt2_plane_js[k][i] = tmprt2[k][jplane][i]; // add xiaolei
			}
		}
	        if (temperature3){
			for (k=zs; k<ze; k++)
			for (i=xs; i<xe; i++) {
				user->tmprt3_plane_js[k][i] = tmprt3[k][jplane][i]; // add xiaolei
			}
		}
			...
			for (k=zs; k<ze; k++)
			for (i=xs; i<xe; i++) {
				user->level_plane_js[k][i] = level[k][jplane][i]; // add xiaolei
			}
		}
        }
	...
	if(rans) DMDAVecRestoreArray(user->fda2, user->K_Omega, &komega);
	if(temperature) DMDAVecRestoreArray(user->da, user->Tmprt, &tmprt); // add xiaolei
	if(temperature1) DMDAVecRestoreArray(user->da, user->Tmprt1, &tmprt1); // add xiaolei
	if(temperature2) DMDAVecRestoreArray(user->da, user->Tmprt2, &tmprt2); // add xiaolei
	if(temperature3) DMDAVecRestoreArray(user->da, user->Tmprt3, &tmprt3); // add xiaolei
	if(levelset) DMDAVecRestoreArray(user->da, user->Levelset, &level); // add xiaolei

        if (!rank) {
		for(k=0; k<mz; k++){ 
			MPI_Reduce(MPI_IN_PLACE, user->ucat_plane_js[k], mx*3, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
		}
                if (rans) {
			for(k=0; k<mz; k++) {
				MPI_Reduce(MPI_IN_PLACE, user->komega_plane_js[k], mx*2, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature) {
			for(k=0; k<mz; k++) {
				MPI_Reduce(MPI_IN_PLACE, user->tmprt_plane_js[k], mx, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature1) {
			for(k=0; k<mz; k++) {
				MPI_Reduce(MPI_IN_PLACE, user->tmprt1_plane_js[k], mx, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature2) {
			for(k=0; k<mz; k++) {
				MPI_Reduce(MPI_IN_PLACE, user->tmprt2_plane_js[k], mx, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature3) {
			for(k=0; k<mz; k++) {
				MPI_Reduce(MPI_IN_PLACE, user->tmprt3_plane_js[k], mx, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}

		if(levelset){
			for(k=0; k<mz; k++) {
 				MPI_Reduce(MPI_IN_PLACE, user->level_plane_js[k], mx, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
	}else{
		for(k=0; k<mz; k++){ 
			MPI_Reduce(user->ucat_plane_js[k], user->ucat_plane_js[k], mx*3, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
		}
                if (rans){
			for(k=0; k<mz; k++) {
				MPI_Reduce(user->komega_plane_js[k], user->komega_plane_js[k], mx*2, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature){ 
			for(k=0; k<mz; k++) {
				MPI_Reduce(user->tmprt_plane_js[k], user->tmprt_plane_js[k], mx, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature1){ 
			for(k=0; k<mz; k++) {
				MPI_Reduce(user->tmprt1_plane_js[k], user->tmprt1_plane_js[k], mx, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature2){ 
			for(k=0; k<mz; k++) {
				MPI_Reduce(user->tmprt2_plane_js[k], user->tmprt2_plane_js[k], mx, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature3){ 
			for(k=0; k<mz; k++) {
				MPI_Reduce(user->tmprt3_plane_js[k], user->tmprt3_plane_js[k], mx, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}

		if(levelset){
			for(k=0; k<mz; k++) {
 				MPI_Reduce(user->level_plane_js[k], user->level_plane_js[k], mx, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
	}
};
...
void store_k_section(UserCtx *user, int kplane)
{
	...
	if ( !ucat_plane_allocated_ks ) {
		ucat_plane_allocated_ks = 1;
		
		user->ucat_plane_ks = (Cmpnts **)malloc( sizeof(Cmpnts *) * my );
		if(rans) user->komega_plane_ks = (Cmpnts2 **)malloc( sizeof(Cmpnts2 *) * my );
		if(temperature) user->tmprt_plane_ks = (double **)malloc( sizeof(double *) * my ); // add xiaolei
		if(temperature1) user->tmprt1_plane_ks = (double **)malloc( sizeof(double *) * my ); // add xiaolei
		if(temperature2) user->tmprt2_plane_ks = (double **)malloc( sizeof(double *) * my ); // add xiaolei
		if(temperature3) user->tmprt3_plane_ks = (double **)malloc( sizeof(double *) * my ); // add xiaolei

		if(levelset) user->level_plane_ks = (double **)malloc( sizeof(double *) * my ); // add xiaolei
		
		for(j=0; j<my; j++) {
			user->ucat_plane_ks[j] = (Cmpnts *)malloc( sizeof(Cmpnts) * mx );
			if(rans) user->komega_plane_ks[j] = (Cmpnts2 *)malloc( sizeof(Cmpnts2) * mx );
			if(temperature) user->tmprt_plane_ks[j] = (double *)malloc( sizeof(double) * mx ); //add xiaolei
			if(temperature1) user->tmprt1_plane_ks[j] = (double *)malloc( sizeof(double) * mx ); //add xiaolei
			if(temperature2) user->tmprt2_plane_ks[j] = (double *)malloc( sizeof(double) * mx ); //add xiaolei
			if(temperature3) user->tmprt3_plane_ks[j] = (double *)malloc( sizeof(double) * mx ); //add xiaolei

			if(levelset) user->level_plane_ks[j] = (double *)malloc( sizeof(double) * mx ); //add xiaolei
		}
	}
	...
	*/
	for(j=0; j<my; j++) 
	for(i=0; i<mx; i++) {
		user->ucat_plane_ks[j][i].x = 0;
                user->ucat_plane_ks[j][i].y = 0;
                user->ucat_plane_ks[j][i].z = 0;
        }
        if (rans) {
	    for(j=0; j<my; j++) 
	    for(i=0; i<mx; i++) {
		user->komega_plane_ks[j][i].x = 0;
                user->komega_plane_ks[j][i].y = 0;
            }
	}
        if (temperature) {
	    for(j=0; j<my; j++) 
	    for(i=0; i<mx; i++) {
		user->tmprt_plane_ks[j][i] = 0;
	    }
	}
        if (temperature1) {
	    for(j=0; j<my; j++) 
	    for(i=0; i<mx; i++) {
		user->tmprt1_plane_ks[j][i] = 0;
	    }
	}
        if (temperature2) {
	    for(j=0; j<my; j++) 
	    for(i=0; i<mx; i++) {
		user->tmprt2_plane_ks[j][i] = 0;
	    }
	}
        if (temperature3) {
	    for(j=0; j<my; j++) 
	    for(i=0; i<mx; i++) {
		user->tmprt3_plane_ks[j][i] = 0;
	    }
	}
        if (levelset) { 
	    for(j=0; j<my; j++) 
	    for(i=0; i<mx; i++) {
		user->level_plane_ks[j][i] = 0;
	    }
        }
	...
	if(levelset) DMDAVecGetArray(user->da, user->Levelset, &level); // add xiaolei
        ...
        {	
	    	for (j=ys; j<ye; j++)
		for (i=xs; i<xe; i++) { 
 			user->ucat_plane_ks[j][i] = ucat[kplane][j][i];
	    	}

        	if (rans) {
			for (j=ys; j<ye; j++)
			for (i=xs; i<xe; i++) {
				user->komega_plane_ks[j][i] = komega[kplane][j][i];
			}
		}
			...
			for (j=ys; j<ye; j++)
			for (i=xs; i<xe; i++) {
				user->tmprt_plane_ks[j][i] = tmprt[kplane][j][i]; // add xiaolei
			}
		}
			...
			for (j=ys; j<ye; j++)
			for (i=xs; i<xe; i++) {
				user->tmprt1_plane_ks[j][i] = tmprt1[kplane][j][i]; // add xiaolei
			}
		}
			...
			for (j=ys; j<ye; j++)
			for (i=xs; i<xe; i++) {
				user->tmprt2_plane_ks[j][i] = tmprt2[kplane][j][i]; // add xiaolei
			}
		}
			...
			for (j=ys; j<ye; j++)
			for (i=xs; i<xe; i++) {
				user->tmprt3_plane_ks[j][i] = tmprt3[kplane][j][i]; // add xiaolei
			}
		}
			...
			for (j=ys; j<ye; j++)
			for (i=xs; i<xe; i++) {
				user->level_plane_ks[j][i] = level[kplane][j][i]; // add xiaolei
			}
		}
	...
	if(levelset) DMDAVecRestoreArray(user->da, user->Levelset, &level); // add xiaolei
        ...
        if (!rank) {
		for(j=0; j<my; j++) {
			MPI_Reduce(MPI_IN_PLACE, user->ucat_plane_ks[j], mx*3, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
		}

                if (rans) {
			for(j=0; j<my; j++) {
				MPI_Reduce(MPI_IN_PLACE, user->komega_plane_ks[j], mx*2, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature) { 
			for(j=0; j<my; j++) {
				MPI_Reduce(MPI_IN_PLACE, user->tmprt_plane_ks[j], mx, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature1) { 
			for(j=0; j<my; j++) {
				MPI_Reduce(MPI_IN_PLACE, user->tmprt1_plane_ks[j], mx, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature2) { 
			for(j=0; j<my; j++) {
				MPI_Reduce(MPI_IN_PLACE, user->tmprt2_plane_ks[j], mx, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature3) { 
			for(j=0; j<my; j++) {
				MPI_Reduce(MPI_IN_PLACE, user->tmprt3_plane_ks[j], mx, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(levelset) {
			for(j=0; j<my; j++) {
 				MPI_Reduce(MPI_IN_PLACE, user->level_plane_ks[j], mx, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
	}else{
		for(j=0; j<my; j++) { 
			MPI_Reduce(user->ucat_plane_ks[j], user->ucat_plane_ks[j], mx*3, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
		}
                if (rans) {
			for(j=0; j<my; j++) {
				MPI_Reduce(user->komega_plane_ks[j], user->komega_plane_ks[j], mx*2, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature) {
			for(j=0; j<my; j++) {
				MPI_Reduce(user->tmprt_plane_ks[j], user->tmprt_plane_ks[j], mx, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature1) {
			for(j=0; j<my; j++) {
				MPI_Reduce(user->tmprt1_plane_ks[j], user->tmprt1_plane_ks[j], mx, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature2) {
			for(j=0; j<my; j++) {
				MPI_Reduce(user->tmprt2_plane_ks[j], user->tmprt2_plane_ks[j], mx, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(temperature3) {
			for(j=0; j<my; j++) {
				MPI_Reduce(user->tmprt3_plane_ks[j], user->tmprt3_plane_ks[j], mx, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
		if(levelset) {
			for(j=0; j<my; j++) {
 				MPI_Reduce(user->level_plane_ks[j], user->level_plane_ks[j], mx, MPI_DOUBLE, MPI_SUM, 0, PETSC_COMM_WORLD);
			}
		}
	}
...
void save_inflow_section(UserCtx *user)
{
	...
	//}
        ...
        for(int i=0; i<nsave_isection; i++) {
                int iplane = save_isection[i];
                if (iplane<1 || iplane>mx-2) continue;

                store_i_section(user, iplane);

                if (!rank) {
			char fname[256];
			int ti_name = ( (ti0-1) / save_inflow_period ) * save_inflow_period + 1;

			sprintf(fname, "%s_i%04d/isection_%06d_dt=%g.dat", path_plane_save, save_isection[i], ti_name, /*user->dt*/dt_inflow);
                        PetscPrintf(PETSC_COMM_WORLD, "%d Saving I-Plane: %d to %s\n", i, iplane, fname); 
			if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
					
			FILE *fp=fopen(fname, "ab");
			if(!fp) printf("\n******************* Cannot open %s ! *******************\n", fname),exit(0);
			
			for(k=0; k<mz; k++) fwrite(&user->ucat_plane_is[k][0], sizeof(Cmpnts), my, fp);
			fclose(fp);
			...
			if(rans) {
				...
				sprintf(fname, "%s_i%04d/isection_ko_%06d_dt=%g.dat", path_plane_save, save_isection[i], ti_name, /*user->dt*/dt_inflow);
				if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
						
				FILE *fp=fopen(fname, "ab");
				if(!fp) printf("\n******************* Cannot open %s ! *******************\n", fname),exit(0);
				
				for(k=0; k<mz; k++) fwrite(&user->komega_plane_is[k][0], sizeof(Cmpnts2), my, fp);
				fclose(fp);
			}
			...
			if(temperature) {
				...
				sprintf(fname, "%s_i%04d/isection_t_%06d_dt=%g.dat", path_plane_save, save_isection[i], ti_name, /*user->dt*/dt_inflow);
				if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
				
				FILE *fp=fopen(fname, "ab");
				if(!fp) printf("\n******************* Cannot open %s ! *******************\n", fname),exit(0);
		
				for(k=0; k<mz; k++) fwrite(&user->tmprt_plane_is[k][0], sizeof(double), my, fp);
				fclose(fp);
			}
			...
			if(temperature1) {
				...
				sprintf(fname, "%s_i%04d/isection_t1_%06d_dt=%g.dat", path_plane_save, save_isection[i], ti_name, /*user->dt*/dt_inflow);
				if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
				
				FILE *fp=fopen(fname, "ab");
				if(!fp) printf("\n******************* Cannot open %s ! *******************\n", fname),exit(0);
		
				for(k=0; k<mz; k++) fwrite(&user->tmprt1_plane_is[k][0], sizeof(double), my, fp);
				fclose(fp);
			}
			...
			if(temperature2) {
				...
				sprintf(fname, "%s_i%04d/isection_t2_%06d_dt=%g.dat", path_plane_save, save_isection[i], ti_name, /*user->dt*/dt_inflow);
				if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
				
				FILE *fp=fopen(fname, "ab");
				if(!fp) printf("\n******************* Cannot open %s ! *******************\n", fname),exit(0);
		
				for(k=0; k<mz; k++) fwrite(&user->tmprt2_plane_is[k][0], sizeof(double), my, fp);
				fclose(fp);
			}
			...
			if(temperature3) {
				...
				sprintf(fname, "%s_i%04d/isection_t3_%06d_dt=%g.dat", path_plane_save, save_isection[i], ti_name, /*user->dt*/dt_inflow);
				if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
				
				FILE *fp=fopen(fname, "ab");
				if(!fp) printf("\n******************* Cannot open %s ! *******************\n", fname),exit(0);
		
				for(k=0; k<mz; k++) fwrite(&user->tmprt3_plane_is[k][0], sizeof(double), my, fp);
				fclose(fp);
			}
			...
			if(levelset) {
				...
				sprintf(fname, "%s_i%04d/isection_l_%06d_dt=%g.dat", path_plane_save, save_isection[i], ti_name, /*user->dt*/dt_inflow);
				if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
				
				FILE *fp=fopen(fname, "ab");
				if(!fp) printf("\n******************* Cannot open %s ! *******************\n", fname),exit(0);
		
				for(k=0; k<mz; k++) fwrite(&user->level_plane_is[k][0], sizeof(double), my, fp);
				fclose(fp);
			}

                  }
        }
        for(int j=0; j<nsave_jsection; j++) {
                int jplane = save_jsection[j];
                if (jplane<1 || jplane>my-2) continue;

                store_j_section(user,jplane);

                if (!rank) {
			char fname[256];
			int ti_name = ( (ti0-1) / save_inflow_period ) * save_inflow_period + 1;

			sprintf(fname, "%s_j%04d/jsection_%06d_dt=%g.dat", path_plane_save, save_jsection[j], ti_name, /*user->dt*/dt_inflow);
                        PetscPrintf(PETSC_COMM_WORLD, "%d Saving J-Plane: %d to %s\n", j, jplane, fname); 
			if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
					
			FILE *fp=fopen(fname, "ab");
			if(!fp) printf("\n******************* Cannot open %s ! *******************\n", fname),exit(0);
			
			for(k=0; k<mz; k++) fwrite(&user->ucat_plane_js[k][0], sizeof(Cmpnts), mx, fp);
			fclose(fp);
			...
			if(rans) {
				...
				int ti_name = ( (ti0-1) / save_inflow_period ) * save_inflow_period + 1;
			        ...
			        sprintf(fname, "%s_j%04d/jsection_ko_%06d_dt=%g.dat", path_plane_save, save_jsection[j], ti_name, /*user->dt*/dt_inflow);
				if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
						
				FILE *fp=fopen(fname, "ab");
				if(!fp) printf("\n******************* Cannot open %s ! *******************\n", fname),exit(0);
				
				for(k=0; k<mz; k++) fwrite(&user->komega_plane_js[k][0], sizeof(Cmpnts2), mx, fp);
				fclose(fp);
			}
			...
			if(temperature) {
				...
				int ti_name = ( (ti0-1) / save_inflow_period ) * save_inflow_period + 1;
			        ...
			        sprintf(fname, "%s_j%04d/jsection_t_%06d_dt=%g.dat", path_plane_save, save_jsection[j], ti_name, /*user->dt*/dt_inflow);
				if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
				
				FILE *fp=fopen(fname, "ab");
				if(!fp) printf("\n******************* Cannot open %s ! *******************\n", fname),exit(0);
		
				for(k=0; k<mz; k++) fwrite(&user->tmprt_plane_js[k][0], sizeof(double), mx, fp);
				fclose(fp);
			}
			if(temperature1) {
				char fname[256];
				int ti_name = ( (ti0-1) / save_inflow_period ) * save_inflow_period + 1;
			        ...
			        sprintf(fname, "%s_j%04d/jsection_t1_%06d_dt=%g.dat", path_plane_save, save_jsection[j], ti_name, /*user->dt*/dt_inflow);
				if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
				
				FILE *fp=fopen(fname, "ab");
				if(!fp) printf("\n******************* Cannot open %s ! *******************\n", fname),exit(0);
		
				for(k=0; k<mz; k++) fwrite(&user->tmprt1_plane_js[k][0], sizeof(double), mx, fp);
				fclose(fp);
			}
			if(temperature2) {
				char fname[256];
				int ti_name = ( (ti0-1) / save_inflow_period ) * save_inflow_period + 1;
			        ...
			        sprintf(fname, "%s_j%04d/jsection_t2_%06d_dt=%g.dat", path_plane_save, save_jsection[j], ti_name, /*user->dt*/dt_inflow);
				if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
				
				FILE *fp=fopen(fname, "ab");
				if(!fp) printf("\n******************* Cannot open %s ! *******************\n", fname),exit(0);
		
				for(k=0; k<mz; k++) fwrite(&user->tmprt2_plane_js[k][0], sizeof(double), mx, fp);
				fclose(fp);
			}
			if(temperature3) {
				char fname[256];
				int ti_name = ( (ti0-1) / save_inflow_period ) * save_inflow_period + 1;
			        ...
			        sprintf(fname, "%s_j%04d/jsection_t3_%06d_dt=%g.dat", path_plane_save, save_jsection[j], ti_name, /*user->dt*/dt_inflow);
				if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
				
				FILE *fp=fopen(fname, "ab");
				if(!fp) printf("\n******************* Cannot open %s ! *******************\n", fname),exit(0);
		
				for(k=0; k<mz; k++) fwrite(&user->tmprt3_plane_js[k][0], sizeof(double), mx, fp);
				fclose(fp);
			}
			...
			if(levelset) {
				...
				int ti_name = ( (ti0-1) / save_inflow_period ) * save_inflow_period + 1;
			        ...
			        sprintf(fname, "%s_j%04d/jsection_l_%06d_dt=%g.dat", path_plane_save, save_jsection[j], ti_name, /*user->dt*/dt_inflow);
				if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
				
				FILE *fp=fopen(fname, "ab");
				if(!fp) printf("\n******************* Cannot open %s ! *******************\n", fname),exit(0);
		
				for(k=0; k<mz; k++) fwrite(&user->level_plane_js[k][0], sizeof(double), mx, fp);
				fclose(fp);
			}
	...
	for(int k=0; k<nsave_ksection; k++) {
		...
		if(!rank) {
			...
			sprintf(fname, "%s_k%04d/inflow_%06d_dt=%g.dat", path_plane_save, save_ksection[k], ti_name, /*user->dt*/dt_inflow);
                        PetscPrintf(PETSC_COMM_WORLD, "%d Saving K-Plane: %d to %s\n", k, kplane, fname); 
			if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
					
			FILE *fp=fopen(fname, "ab");
			if(!fp) printf("\n******************* Cannot open %s ! *******************\n", fname),exit(0);
			
			for(j=0; j<my; j++) fwrite(&user->ucat_plane_ks[j][0], sizeof(Cmpnts), mx, fp);
			fclose(fp);
			...
			if(rans) {
				...
				// sprintf(fname, "%s/k%04d_inflow_ko_%06d_dt=%g.dat", path, kplane, ti_name, /*user->dt*/dt_inflow);
			        sprintf(fname, "%s_k%04d/inflow_ko_%06d_dt=%g.dat", path_plane_save, save_ksection[k], ti_name, /*user->dt*/dt_inflow);
				if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
						
				FILE *fp=fopen(fname, "ab");
				if(!fp) printf("\n******************* Cannot open %s ! *******************\n", fname),exit(0);
				
				for(j=0; j<my; j++) fwrite(&user->komega_plane_ks[j][0], sizeof(Cmpnts2), mx, fp);
				fclose(fp);
			}
			...
			if(temperature) {
				...
				int ti_name = ( (ti0-1) / save_inflow_period ) * save_inflow_period + 1;
			        ...
			        sprintf(fname, "%s_k%04d/inflow_t_%06d_dt=%g.dat", path_plane_save, save_ksection[k], ti_name, /*user->dt*/dt_inflow);
				if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
				
				FILE *fp=fopen(fname, "ab");
				if(!fp) printf("\n******************* Cannot open %s ! *******************\n", fname),exit(0);
		
				for(j=0; j<my; j++) fwrite(&user->tmprt_plane_ks[j][0], sizeof(double), mx, fp);
				fclose(fp);
			}
			...
			if(temperature1) {
				...
				int ti_name = ( (ti0-1) / save_inflow_period ) * save_inflow_period + 1;
			        ...
			        sprintf(fname, "%s_k%04d/inflow_t1_%06d_dt=%g.dat", path_plane_save, save_ksection[k], ti_name, /*user->dt*/dt_inflow);
				if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
				
				FILE *fp=fopen(fname, "ab");
				if(!fp) printf("\n******************* Cannot open %s ! *******************\n", fname),exit(0);
		
				for(j=0; j<my; j++) fwrite(&user->tmprt1_plane_ks[j][0], sizeof(double), mx, fp);
				fclose(fp);
			}
			...
			if(temperature2) {
				...
				int ti_name = ( (ti0-1) / save_inflow_period ) * save_inflow_period + 1;
			        ...
			        sprintf(fname, "%s_k%04d/inflow_t2_%06d_dt=%g.dat", path_plane_save, save_ksection[k], ti_name, /*user->dt*/dt_inflow);
				if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
				
				FILE *fp=fopen(fname, "ab");
				if(!fp) printf("\n******************* Cannot open %s ! *******************\n", fname),exit(0);
		
				for(j=0; j<my; j++) fwrite(&user->tmprt2_plane_ks[j][0], sizeof(double), mx, fp);
				fclose(fp);
			}
			...
			if(temperature3) {
				...
				int ti_name = ( (ti0-1) / save_inflow_period ) * save_inflow_period + 1;
			        ...
			        sprintf(fname, "%s_k%04d/inflow_t3_%06d_dt=%g.dat", path_plane_save, save_ksection[k], ti_name, /*user->dt*/dt_inflow);
				if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
				
				FILE *fp=fopen(fname, "ab");
				if(!fp) printf("\n******************* Cannot open %s ! *******************\n", fname),exit(0);
		
				for(j=0; j<my; j++) fwrite(&user->tmprt3_plane_ks[j][0], sizeof(double), mx, fp);
				fclose(fp);
			}
			...
			if(levelset) {
				...
				int ti_name = ( (ti0-1) / save_inflow_period ) * save_inflow_period + 1;
			        ...
			        sprintf(fname, "%s_k%04d/inflow_l_%06d_dt=%g.dat", path_plane_save, save_ksection[k], ti_name, /*user->dt*/dt_inflow);
				if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
				
				FILE *fp=fopen(fname, "ab");
				if(!fp) printf("\n******************* Cannot open %s ! *******************\n", fname),exit(0);
		
				for(j=0; j<my; j++) fwrite(&user->level_plane_ks[j][0], sizeof(double), mx, fp);
				fclose(fp);
			}
...
//Add-Hossein for savekplanes (again put it back to 3.1)
...
void store_k_CrossSection(UserCtx *user)
{
	DMDALocalInfo info = user->info;
	int	xs = info.xs, xe = info.xs + info.xm;
	int 	ys = info.ys, ye = info.ys + info.ym;
	...
	PetscPrintf(PETSC_COMM_WORLD, "mz and kplane: %d %d\n", mz,kplane);
...
void Calc_ShearStress(UserCtx *user)
{
	...
	if (user->bctype[2]==-1 || user->bctype[2]==-2) {	// wall function
		if (ys==0) {
			j = 1;
			for (k=lzs; k<lze; k++)
			for (i=lxs; i<lxe; i++) {
	...
	if (user->bctype[3]==-1 || user->bctype[3]==-2 ) {	// wall function
		if (ye==my) {
			j = my-2;
			for (k=lzs; k<lze; k++)
			for (i=lxs; i<lxe; i++) {
	...
	for(i=0; i<6; i++) {
		MPIU_Allreduce(&lforce[i], &force[i], 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
		MPIU_Allreduce(&lforce_skin[i], &force_skin[i], 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
		MPIU_Allreduce(&lforce_pressure[i], &force_pressure[i], 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
		MPIU_Allreduce(&lforce_avg[i], &force_avg[i], 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
		MPIU_Allreduce(&larea[i], &area[i], 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
	...
	if(immersed) {
		MPIU_Allreduce(&lforce_ibm, &force_ibm, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
		MPIU_Allreduce(&lforce_avg_ibm, &force_avg_ibm, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
		MPIU_Allreduce(&larea_ibm, &area_ibm, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
	...
	*/
	...
	if(inletprofile==17) {
		...
		MPIU_Allreduce(&sum_error_u, &total_sum_error_u, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
		MPIU_Allreduce(&sum_error_v, &total_sum_error_v, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
		MPIU_Allreduce(&sum_error_p, &total_sum_error_p, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
		
		MPIU_Allreduce(&inf_u, &total_inf_u, 1, MPIU_REAL, MPIU_MAX, PETSC_COMM_WORLD);
		MPIU_Allreduce(&inf_v, &total_inf_v, 1, MPIU_REAL, MPIU_MAX, PETSC_COMM_WORLD);
		MPIU_Allreduce(&inf_p, &total_inf_p, 1, MPIU_REAL, MPIU_MAX, PETSC_COMM_WORLD);
...
void Calc_k_Flux(UserCtx *user)
{
	...
	*/
					...
					if(j>=1 && j<=my-2 && i>=1 && i<=mx-2) {
					  ...
					  if( levelset ) {
					    ...
					    double vf = 1.0;//H(level[k+1][j][i], dx); TEST Toni 
							if(air_flow_levelset_periodic && level[k][j][i]>-dthick)vf=0.; //Hossein
	...
	MPIU_Allreduce(&lFlux, &Flux, 1, MPIU_REAL, MPIU_SUM, PETSC_COMM_WORLD);
	/*
