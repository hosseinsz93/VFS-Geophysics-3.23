...
PetscErrorCode FSI_DATA_Input(FSInfo *FSinf, PetscInt ti)
{
  ...
  f = fopen(filen, "r");
  if (!f) {
    SETERRQ(PETSC_COMM_SELF, PETSC_ERR_FILE_OPEN, "Cannot open IBM node file");
...
int main(int argc, char **argv)
{
  ...
  PetscOptionsGetInt(NULL, NULL, "-tis", &tistart, &flg);
  PetscOptionsGetInt(NULL, NULL, "-tie", &tiend, &flg);
