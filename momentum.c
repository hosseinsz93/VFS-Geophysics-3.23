...
extern PetscInt immersed, inviscid, density_current;
...
double Calc_Minimum_dt (UserCtx *user)
{
	...
	MPIU_Allreduce(&ldt, &dt, 1, MPIU_REAL, MPIU_MIN, PETSC_COMM_WORLD);
	MPIU_Allreduce(&ldx, &dx_min, 1, MPIU_REAL, MPIU_MIN, PETSC_COMM_WORLD);
	MPIU_Allreduce(&ldi_min, &di_min, 1, MPIU_REAL, MPIU_MIN, PETSC_COMM_WORLD);
	MPIU_Allreduce(&ldj_min, &dj_min, 1, MPIU_REAL, MPIU_MIN, PETSC_COMM_WORLD);
	MPIU_Allreduce(&ldk_min, &dk_min, 1, MPIU_REAL, MPIU_MIN, PETSC_COMM_WORLD);
	
	MPIU_Allreduce(&ldi_max, &di_max, 1, MPIU_REAL, MPIU_MAX, PETSC_COMM_WORLD);
	MPIU_Allreduce(&ldj_max, &dj_max, 1, MPIU_REAL, MPIU_MAX, PETSC_COMM_WORLD);
	MPIU_Allreduce(&ldk_max, &dk_max, 1, MPIU_REAL, MPIU_MAX, PETSC_COMM_WORLD);
...
void Pressure_Gradient(UserCtx *user, Vec dP)
{
  ...
  /* First we calculate the flux on cell surfaces. Stored on the upper integer
	...
	DMGlobalToLocalBegin(user->da, user->P, INSERT_VALUES, user->lP); //Hossein tempoarily made it comment
...
//	for wave_momentum_source
	...
	if(wave_momentum_source || wave_sponge_layer) DMDAVecGetArray(fda, user->lWAVE_fp, &WAVE_fp);	
// end (Toni)
	...
	*/
	...
	for (i=xs; i<xe; i++) {
		...
		if ( immersed && i!=mx-2 && nvert[k][j][i]>0.1) {
		  ...
		  if( inviscid || (rans && !central) || (levelset && les && lm < dthick ) ) {
			  if(1==0) {
				div1[k][j][i].x = 
	...
	for (i=xs; i<xe; i++) {
		...
		*/
		...
		DMLocalToLocalBegin(fda, Div1, INSERT_VALUES, Div1);
		DMLocalToLocalEnd(fda, Div1, INSERT_VALUES, Div1);
		DMLocalToLocalBegin(fda, Div2, INSERT_VALUES, Div2);
		DMLocalToLocalEnd(fda, Div2, INSERT_VALUES, Div2);
		DMLocalToLocalBegin(fda, Div3, INSERT_VALUES, Div3);
		DMLocalToLocalEnd(fda, Div3, INSERT_VALUES, Div3);
	...
	if(skew) {
		...
		DMLocalToLocalBegin(fda, Adv1, INSERT_VALUES, Adv1);
		DMLocalToLocalEnd(fda, Adv1, INSERT_VALUES, Adv1);
		DMLocalToLocalBegin(fda, Adv2, INSERT_VALUES, Adv2);
		DMLocalToLocalEnd(fda, Adv2, INSERT_VALUES, Adv2);
		DMLocalToLocalBegin(fda, Adv3, INSERT_VALUES, Adv3);
		DMLocalToLocalEnd(fda, Adv3, INSERT_VALUES, Adv3);
	...
	DMLocalToLocalBegin(fda, Visc1, INSERT_VALUES, Visc1);
	DMLocalToLocalEnd(fda, Visc1, INSERT_VALUES, Visc1);
	DMLocalToLocalBegin(fda, Visc2, INSERT_VALUES, Visc2);
	DMLocalToLocalEnd(fda, Visc2, INSERT_VALUES, Visc2);
	DMLocalToLocalBegin(fda, Visc3, INSERT_VALUES, Visc3);
	DMLocalToLocalEnd(fda, Visc3, INSERT_VALUES, Visc3);
	...
	DMLocalToLocalBegin(fda, Fp, INSERT_VALUES, Fp);
	DMLocalToLocalEnd(fda, Fp, INSERT_VALUES, Fp);
...
*/
	...
	// add (Toni)
	//for wave_momentum_source
	if(wave_momentum_source || wave_sponge_layer) DMDAVecRestoreArray(fda, user->lWAVE_fp, &WAVE_fp);	 
...
PetscErrorCode FormFunction_SNES(SNES snes, Vec Ucont, Vec Rhs, void *ptr)
{
	...
	VecAXPY(Rhs, -1, user->dP);
		...
		if (nacelle_model || rotor_model) VecAXPY(Rhs, 1., user->F_eul);
    
//Meric
		//PetscPrintf(PETSC_COMM_WORLD, "Adding Canopy Force");
		if (cnpy) VecAXPY(Rhs, 1.0, user->cnpyF);

         if(density_current == 1 || density_current == 2) VecAXPY (Rhs, -1, user->FCurrent); //For Air or Temprature it should be 1 because the higher temprature goes up, but the higher density goes down
         if(density_current == 3) VecAXPY (Rhs, 1, user->FCurrent); //For Air or Temprature it should be 1 because the higher temprature goes up, but the higher density goes down
