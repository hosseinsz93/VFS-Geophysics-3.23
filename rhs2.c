...
void save_inflow_section(UserCtx *user)
{
	...
	if(!rank) {
		...
		sprintf(fname, "%s%04d/inflow_%06d_dt=%g.dat", path_inflow, ti_name, user->dt);
	...
	if(!rank && rans) {
		...
		sprintf(fname, "%s%04d/inflow_ko_%06d_dt=%g.dat", path_inflow, ti_name, user->dt);
		if(ti0==tistart || ti0%save_inflow_period==1) unlink(fname);	// delete existing file
...
void read_inflow_section(UserCtx *user)
{
	...
	sprintf(fname, "%s/inflow_%06d_dt=%g.dat", path_inflow, ti_name, user->dt);
	...
	if(rans) {
		if(!rank) {
			...
			sprintf(fname, "%s/inflow_ko_%06d_dt=%g.dat", path_inflow, ti_name, user->dt);
...
void write_shear_stress_ibm()
{
	for(int ibi=0; ibi<NumberOfBodies; ibi++) {
		...
		if(!my_rank) {
			...
			sprintf(filen, "%s/shear%06d_%2.2d.dat",path_inflow,ti,ibi);
			fp = fopen(filen, "w");
